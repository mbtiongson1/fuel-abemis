...
\begin{enumerate}
  ...
  \item \textbf{Train/Test Split} --- An 80/20 stratified split (376 total records; 300 training, 76 test) is applied. Stratification is on \texttt{machinery\_family}.
...
\begin{itemize}
  \item \textbf{Scanned PDFs:} Tesseract 5.5.2 OCR was applied to 267 previously LOW\_TEXT or unreadable AMTEC reports during this study, recovering 24 additional comparable records (corpus expanded from 371 legacy + 5 newly text-extracted to 376). The remaining unrecovered reports are predominantly 1990s placeholder filings without machine specifications even after OCR. Acquisition of additional source PDFs (notably the previously inaccessible 3{,}065-PDF collaborator set) would further enlarge the corpus.
